...
in the encoding. It is impossible to access them from the `dia_get` function, but the `dia_sum` uses them. 
...
Lemma dia_sum_spec {D : Type} `{Inhab D} (d_ind d_val : loc):
    ...
    {{ compare results from (1, (0,0)) with the predefined sum of individual values in (2, (i, j)) \* Arrs}}
    *)
Proof.
(* 
    ...
    -  The first premise is proved by Fact 1 below.
    ...
    simplifying remaining goal and framing out v₂ = 0
